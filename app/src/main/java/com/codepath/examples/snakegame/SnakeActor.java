...
public class SnakeActor extends SimpleMovingActor {
    ...
    public void handleKeyInput(int keyCode) {
        if (keyCode == KeyEvent.KEYCODE_W) {
            ...
            getVelocity().stop().setXDirection(Velocity.DIRECTION_LEFT).setYSpeed(STEP);
        } else if (keyCode == KeyEvent.KEYCODE_D) {
            getVelocity().stop().setXDirection(Velocity.DIRECTION_RIGHT).setYSpeed(STEP);
        }
    }
    public void handleKeyInput(MotionEvent event) {
